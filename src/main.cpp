...
int main() {
	...
	// Lane width. In a real world scenario, might be useful to keep this located in a variable, 
	// so dynamic lane widths can be taken into account and have the path planner readjust on the fly.
	int lane_width = 4;
	...
	// Acceleration steps
	double ref_vel_accel = 0.225;

	// Max speed (just barely under the limit)
	double max_speed = 49.5;

	// Path planner size
	int path_planner_size = 50;

  	h.onMessage([&map_waypoints_x, &map_waypoints_y, &map_waypoints_s, &map_waypoints_dx, &map_waypoints_dy, &lane, &ref_vel, ref_vel_accel, max_speed, lane_width, path_planner_size](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length, uWS::OpCode opCode) {
		...
		if (length && length > 2 && data[0] == '4' && data[1] == '2') 
		{
			...
			if (s != "") 
			{
				...
				if (event == "telemetry") 
				{
					...
					// Sensor fusion data obtained from j
					...
					for (int i = 0; i < sensor_fusion.size(); i++) 
					{
						...
						// Control speed and car position, based on sensor fusion data
					...
					// Speed management to prevent jerkiness and avoid head-on collision
					if (too_close)
					{
						ref_vel -= ref_vel_accel;
					} 
					...
					// Frenet transformation to x,y coordinates. By passing in the lane variable 
					// we're implementing lane changes using spline for smooth driving
					...
					// Spline initialization and x,y set
					tk::spline s;
					s.set_points(ptsx,ptsy);

					// Actual points for the path planner
					...
					// Spline management so we can travel at the desired velocity
					double target_x = 30.0;
					double target_y = s(target_x);
					double target_dist = sqrt((target_x)*(target_x)+(target_y)*(target_y));
					double x_add_on = 0;

					// Loop in order to fill the path planner
					for (int i = 1; i <= path_planner_size - previous_path_x.size(); i++) 
					{
						...
						// Going back to normal coordinates by shift and rotate
